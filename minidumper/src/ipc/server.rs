...
impl Server {
    ...
    fn handle_crash_request(
        ...
        let (mut minidump_file, minidump_path) = handler.create_minidump_file()?.unzip();
        #[cfg(not(target_os = "windows"))]
        let mut minidump_file_cursor: Cursor<Vec<u8>> = Cursor::new(Vec::new());
        ...
        #[cfg(not(target_os = "windows"))]
        let result = writer.dump(&mut minidump_file_cursor);
        if let Some(minidump_file) = minidump_file.as_mut() {
            #[cfg(not(target_os = "windows"))]
            {
                let buffer = &minidump_file_cursor.clone().into_inner();
                minidump_file.write_all(buffer).unwrap();
                minidump_file.flush().unwrap();
            }
            #[cfg(target_os = "windows")]
            {
                result = result.and_then(
                    |buffer| -> Result<Vec<u8>, minidump_writer::errors::Error> {
                        minidump_file.write_all(&buffer)?;
                        Ok(buffer)
                    },
                );
            }
